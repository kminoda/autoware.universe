...
: node_(node),
  ...
  converged_param_type_(ConvergedParamType::TP),
  ...
  regularization_enabled_(node->declare_parameter("regularization_enabled", false))
{
  ...
  NDTImplementType ndt_type = (*ndt_ptr_ptr_)->getImplementationType();
  if (
    ndt_type != NDTImplementType::OMP && ndt_type != NDTImplementType::OMP_MULTI_VOXEL &&
    converged_param_type_ == ConvergedParamType::NVTL) {
    RCLCPP_ERROR(
      logger_,
      "ConvergedParamType::NVTL is only available when "
      "NDTImplementType::OMP or OMP_MULTI_VOXEL is selected.");
    return;
  }
  ...
  service_trigger_node_ = node->create_service<std_srvs::srv::SetBool>(
    "trigger_node_srv",
    std::bind(
      &NDTScanMatchingModule::serviceTriggerNode, this, std::placeholders::_1, std::placeholders::_2),
  ...
  const geometry_msgs::msg::PoseWithCovarianceStamped::ConstSharedPtr initial_pose_msg_ptr)
{
  ...
  if (initial_pose_msg_ptr->header.frame_id == map_frame_) {
    ...
    tf2_listener_module_->get_transform(clock_->now(), map_frame_, initial_pose_msg_ptr->header.frame_id, TF_pose_to_map_ptr);
  ...
  sensor_msgs::msg::PointCloud2::ConstSharedPtr sensor_points_sensorTF_msg_ptr)
{
  ...
  if (regularization_enabled_ && ((*ndt_ptr_ptr_)->getImplementationType() == NDTImplementType::OMP))
  ...
  const NdtResult ndt_result = (*ndt_ptr_ptr_)->align(interpolator.get_current_pose().pose.pose);
  ...
  pcl::shared_ptr<pcl::PointCloud<PointSource>> sensor_points_output_ptr)
{
  ...
  tf2_listener_module_->get_transform(clock_->now(), target_frame, source_frame, TF_target_to_source_ptr);
  ...
  const Eigen::Matrix4f base_to_sensor_matrix =
    tier4_autoware_utils::poseToMatrix4f(target_to_source_pose_stamped.pose);
  pcl::transformPointCloud(
  ...
  const rclcpp::Time & sensor_ros_time)
{
  ...
  return tier4_autoware_utils::poseToMatrix4f(interpolator.get_current_pose().pose.pose);
}
...
void NDTScanMatchingModule::serviceTriggerNode(
